...
const saveTaskWork = async(req, res) => {
    ...
    if (!workFind)
        res.status(400).send({ message: "work not found" });
...
const listBoardByIdWork = async(req, res) => {
    ...
    return boardList.length === 0 ? res.status(400).send({ message: "You no have work board assigned" }) : res.status(200).send({ boardList });
};
...
export default { saveTaskImg, listTask, updateTask, deleteTask, saveTaskWork, listBoardByIdWork };